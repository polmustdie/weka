...
public class OneClassSVM {
	...
	/** These data set contains only <b>ok</b> as well as <b>?</b> */
	private static String[] dataPaths = {"src/main/resources/test.arff"};

	/** Project names array*/
	private static String[] proNames = {"test"};
	...
	public static void evaluateTest(Instances okIns, Instances testIns, Classifier cls) throws Exception{

		Instances newTest = changeLabelTook(testIns);
...
//			System.out.println(">>" + ins.getOriginLabel());
		...
		cls.buildClassifier(okIns);
		Evaluation eval= new Evaluation(okIns);
...
//		System.out.println("TN: " + tn);
	...
	public static void splitNormal(String path){
		...
		for(Instance ins: origin){
			if(ins.stringValue(numAttr-1).equals("ok")){
				normal.add(ins);
			}
		}
			
		/************** Change the Class label from {no,ok} to {ok}  ****************/
		normal = changeLabelTook(normal);
		
		/************** Write the normal dataset *************/
		System.out.println(path + ": " + normal.size()); // print the normal data point number
		
		try {
			DataSink.write("src/main/resources/" + selectName(path) + "-ok.arff", normal); //save the normal instances
		} catch (Exception e) {
	...
	/** Change the Class label from {no,ok} to {ok} */
	public static Instances changeLabelTook(Instances origin){
...
class INS{
	...
	public void setFlag(){
		if(this.orLabel.equals("no")&&this.deLabel.equals("no")){
			this.flag = "TP";
		}else if(this.orLabel.equals("no")&&this.deLabel.equals("ok")){
			this.flag = "FN";
		}else if(this.orLabel.equals("ok")&&this.deLabel.equals("no")){
			this.flag = "FP";
		}else{
			this.flag = "TN";
		}
